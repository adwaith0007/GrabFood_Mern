...
const MenuPage = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await api.get(
        `/product/get/user?category=${selectedCategory}&page=${currentPage}&limit=${rowsPerPage}`
      );
      if (response.data.success) {
  ...
  return (
    <div className="bg-[#e5d9ca] h-full min-h-screen ">
      <div className="container custom-height flex flex-col justify-center items-center mx-auto  ">
        ...
        <div className="hidden lg:flex lg:flex-wrap justify-center gap-4 mt-8">
          {categoriesLoading
        ...
        <div className="w-full h-full border-t mt-5 border-[#9a9b9e] flex justify-between ">
          <div className="relative inline-block text-left">
            ...
            {sortBtn && (
              <div
                className="absolute left-0 ml-2 z-20 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                role="menu"
          ...
          {loadingProducts
            ? Array.from({ length: 8 }).map((_, index) => (
                <div className="flex" key={index}>
                  <div className="relative m-10 flex w-[297px] max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
                    <div className="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl">
                      <Skeleton className="rounded" width={278} height={249} />
                    </div>
                    ...
                    <div className="mt-4 px-5 pb-5">
                      ...
                      <Skeleton
                        ...
                        className=" w-full  rounded-md  px-5 py-2.5  "
                      />
                    </div>
                  </div>
                </div>
...
//     setLoadingProducts(true);
//     try {
//       const response = await api.get(`/product/get/user?category=${selectedCategory}&page=${currentPage}&limit=${rowsPerPage}`);
//       if (response.data.success) {
//         setProductList(response.data.data);
//         setTotalPages(response.data.totalPages);
//         setLoadingProducts(false);
...
//   const HandleBestProduct = async () => {
//     try {
//       const res = await api.get(`/product/get_best?category=${selectedCategory}&page=${currentPage}&limit=${rowsPerPage}`);
//       if (res.data.success) {
//         setProductList(res.data.data);
//         setTotalPages(res.data.totalPages);
//       }
...
//   const searchProducts = (query) => {
//     try {
//       api.get(`/product/search?q=${query}&page=${currentPage}&limit=${rowsPerPage}`).then((res) => {
//         if (res.data.success) {
//           setProductList(res.data.data);
//           setTotalPages(res.data.totalPages);
//         }
...
//         <div className="h-full border-t mt-5 border-[#9a9b9e] flex justify-between ">
//           <div className="relative inline-block text-left">
//             <div>
//               <button
//                 onClick={() => setSortBtn(!sortBtn)}
...
//       <div className="flex justify-center pt-2 pb-10  "><nav aria-label="Page navigation example">
//   <ul className="inline-flex -space-x-px text-base h-10">
//     <li>
//       <button
//         onClick={() =>
