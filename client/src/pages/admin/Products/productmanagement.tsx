...
//                 >
//                   {productDetails?.deleted ? "Undo Deleted" : "Soft Delete"}
//                 </button>
//               </div>
//             </div>
...
//   const handleSoftDelete = async (): Promise<void> => {
//     try {
//       const response = await api.put(`/product/softDelete/${id}`);
//       if (response.data.success) {
//         setProductDetails((prevProduct) => ({
//           ...prevProduct,
//           deleted: !prevProduct.deleted,
//         }));
//         toast.success(`Product ${productDetails.deleted ? "restored" : "soft deleted"} successfully!`);
//       } else {
//         toast.error("Failed to delete Product");
//       }
//     } catch (error) {
...
//                   disabled={updating}
//                 >
//                   {productDetails?.deleted ? "Undo Delete" : "Soft Delete"}
//                 </button>
//               </div>
//             </div>
//             <button type="submit" disabled={updating}>
...
// the above code is working fine remove the image crop
...
//     setOffer(0);
//     setProductDetails({ ...productDetails, discountPrice: null });
//     setAddOfferBox(false);
//   };
...
const ProductManagement = () => {
  ...
  const handleCancelOffer = async () => {
    try {
      ...
      const updatedProductDetails = { ...productDetails, discountPrice: null };
      setProductDetails(updatedProductDetails);

      const formData = new FormData();
      formData.append("name", updatedProductDetails.productName);
      formData.append("price", String(updatedProductDetails.price));
      formData.append("category", updatedProductDetails.category);
      formData.append("offer", "0");

      updatedProductDetails.productImage.forEach((image: string) => {
        formData.append("images", image);
      });
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Failed to cancel offer");
      }
    } catch (error) {
